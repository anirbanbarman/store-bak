...
@Component({
  selector: "app-itemreceive",
  templateUrl: "./itemreceive.component.html",
  styleUrls: ["./itemreceive.component.scss"],
  providers: [DatePipe]
})
export class ItemreceiveComponent implements OnInit,OnDestroy {
  ...
  getItemAll() {
    ...
    this.dashboardService.StoreitemList().subscribe((response: any) => {
      if (response && response.status === 200 && response.data) {
        ...
        {
          console.log('info[i].description',info[i].description)
  ...
  {  
    ...
    const scriptURL = 'https://script.google.com/macros/s/AKfycby2tFy-YeAKkZBMPtRvfSHsKirQ622NPVtLBhl5KvECBFjv0hTEiQeOFAGU8qN2KjvmTQ/exec'
  ...
  save(){ 
    ...
    else
    {
      if(this.isNew==true)
      {
        ...
        this.dashboardService.saveItemreceives(payload).subscribe((response: any) => {
  ...
  delete()
  {
   ...
   // console.log('this.currentDateTime',this.currentDateTime);
    ...
    this.dashboardService.deleteItemreceives(payload).subscribe((response: any) => {
      if (response && response ?.status === 200) {
       //console.log('response', response.data);
       // console.log('response.id', response.data.id);
        this.spinner.hide();
  ...
  update()
  {
    if(this.isNew==false)
    {
     ...
     // console.log('updateWeightscale');
      ...
      this.dashboardService.updateItemreceives(payload).subscribe((response: any) => {
        if (response && response ?.status === 200) {
         // console.log('response', response.data);
         // console.log('response.id', response.data.id);
          this.spinner.hide();
  ...
  openItem(item:any)
  {
     ...
     //console.log('weightscale',weightscale[0])
      ...
      this.data.Supplier=data[0].Supplier;
  ...
  getDataAll() {
    this.outputList=[];
    this.spinner.show()
    this.dashboardService.ItemreceiveList().subscribe((response: any) => {
      if (response && response.status === 200 && response.data) {
        this.spinner.hide();
        const info = response.data;
